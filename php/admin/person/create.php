...
if (!$mysqli = getConectionDb()) {
    ...
    if ($result) {
        if ($result->num_rows > 0) {
            ...
            $insertSql = "INSERT INTO personas (id_empresa, cedula, nombres, apellidos, id_genero, fecha_nacimiento, id_tipo_licencia, conyugue, direccion, correo, celular, imagen) "
                    . "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            $stmt = $mysqli->prepare($insertSql);
            if ($stmt) {
                $stmt->bind_param("isssisisssss", $idCompany, $json["documentPerson"], utf8_decode($json["namePerson"]), utf8_decode($json["surnamePerson"]), $json["idGenderPerson"], $json["dateOfBirthPerson"], $json["idTypeLicensePerson"], utf8_decode($json["spousePerson"]), utf8_decode($json["addressPerson"]), $json["emailPerson"], $json["cellPerson"], utf8_decode($json["imagePerson"]));
                $stmt->execute();
